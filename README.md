...
一個用 Three.js 打造的 3D 進擊的巨人互動小遊戲：**先通過牆外特別調查任務的故事開場（密碼解鎖）**，再操控 **艾連** 繞到 **鎧之巨人** 背後，精準斬擊**後頸弱點**！擊敗巨人後，前往 **實驗室風格勝利畫面**，為心愛的人兌換專屬禮物！
...
巨人倒地（蒸氣四散）→ 🎁 **3D 禮物盒** 從天而降 → 靠近禮物 → 蓋子飛開、光柱升起 → 🔬 自動前往**實驗室風格勝利報告**（VICTORY SCREEN）：戰利品「聖明之水・樂敦 me+me 水光瞳」＋9 月配送資訊！
...
# 瀏覽器開 http://localhost:8000  → 故事開場（首頁 index.html）
#       http://localhost:8000/game.html → 直接進遊戲
#       http://localhost:8000/victory.html → 直接看勝利畫面
```
...
- **原生 HTML5 + CSS3 + ES6**，三頁分工：`index.html`（故事開場）→ `game.html`（遊戲）→ `victory.html`（勝利畫面），無 build 步驟
...
`game.html`（遊戲）內建除錯介面（console 可用），方便自動化測試：
